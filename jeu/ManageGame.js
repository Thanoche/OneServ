...
module.exports = class ManageGame {
  ...
  GameStart() {
    ...
    console.log(
      "Le joueur " + this.currentPlayer.name + " tu vas commencer à jouer"
    );
  }
      ...
      if (isPlayableCard) {
        ...
        if (
          ...
          ((card.getColor() == this.lastCard.color &&
            card.getValue() != this.lastCard.value) ||
            (card.getColor() == this.lastColor &&
              card.getValue() != this.lastCard.value))
        ) {
          if (card.isPlus2Card()) {
            this.sumPinition += 2;
          ...
          if (!playedSameValue && card.isChangeColorCard()) {
            ...
            if (!playedSameValue && card.isPlus4Card()) {
              ...
              if (
                card.getValue() === this.lastCard.value ||
                (card.isPlus2Card() && this.lastCard.isPlus4Card())
              ) {
                if (card.isPlus4Card()) {
                  this.sumPinition += 4;
  ...
  reverseGameDirection() {
    ...
    for (let currentPlayer of this.players) {
      ...
      [currentPlayer.nextPlayer, currentPlayer.previousPlayer] = [
  ...
  notOne(player) {
    this.UnoDeck.dealCards([player], 2, this.lastCard);
  }
  ...
  skipNextPlayer() {
    ...
    console.log(
      this.currentPlayer.name + " tu ne peux pas jouer, tu es sauté !! "
    );
  ...
  async playTurn() {
    ...
    console.log(
      `${this.currentPlayer.name}, voici ta main: `,
  ...
  getColorChoice() {
    return new Promise((resolve, reject) => {
      ...
      rl.question(
        "Veuillez choisir une couleur (violet, bleu, rose, vert) : ",
        (color) => {
          rl.close();
          ...
          if (validColors.includes(color)) {
            ...
            reject(
              new Error(
                "Couleur invalide ! Veuillez choisir parmi : rouge, bleu, vert, jaune."
              )
            );
          }
        }
  ...
  async executeGame() {
    ...
    console.log(
      "Le jeu est terminé! Le joueur " + this.currentPlayer.name + " a gagné!"
    );
  }
